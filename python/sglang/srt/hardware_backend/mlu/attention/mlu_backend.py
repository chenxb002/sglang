...
class MLUAttnBackend(AttentionBackend):
    ...
    @staticmethod
    def _raise_unsupported_mla_cache_path(path: str):
        raise RuntimeError(
            "MLUAttnBackend does not support MLA cached-prefix attention in "
            f"{path}. Use MLUMLABackend for MLA models instead; falling back here "
            "would require a whole-cache contiguous copy."
        )

    def _init_layer_forward_metadata(
        self,
        layer: RadixAttention,
        k_rope: Optional[torch.Tensor],
    ) -> None:
        meta = getattr(self, "forward_metadata", None)
        if meta is None:
            return
        meta.is_mla = k_rope is not None or layer.qk_head_dim != layer.v_head_dim
        meta.head_size_v = layer.v_head_dim if meta.is_mla else -1
    ...
    def _init_cuda_graph_forward_metadata(
        ...
        metadata.seq_lens = forward_batch.seq_lens
        metadata.max_seq_len_q = 1
        metadata.max_seq_len_kv = int(forward_batch.seq_lens_cpu[:bs].max().item())

        max_seq_pages = (
            metadata.max_seq_len_kv + self.page_size - 1
        ) // self.page_size
        if max_seq_pages > 0:
    ...
    def _init_eager_forward_metadata(self, forward_batch: ForwardBatch):
        ...
        if mode == ForwardMode.EXTEND:
            ...
            batch_size = forward_batch.batch_size
            mix_running_indices = getattr(forward_batch, "mix_running_indices", None)
            if mix_running_indices is None:
                raise RuntimeError(
                    "MLU mixed attention requires ForwardBatch.mix_running_indices."
                )
            running_bs = len(mix_running_indices)
    ...
    def forward(
        ...
        **kwargs,
        ...
        mode = forward_batch.forward_mode
        self._init_layer_forward_metadata(layer, kwargs.get("k_rope"))
        if mode.is_idle():
    ...
    def forward_extend(
        ...
        if k_rope is not None:
            # k already reshaped by RadixAttention to (-1, tp_k_head_num, v_head_dim)
            k = k.view(-1, layer.tp_k_head_num, layer.v_head_dim)
            k_rope = k_rope.view(
                -1, layer.tp_k_head_num, layer.qk_head_dim - layer.v_head_dim
            )
        else:
        ...
        if save_kv_cache:
            if k_rope is not None:
                cache_k = torch.cat([k, k_rope], dim=-1)
                self.token_to_kv_pool.set_kv_buffer(
                    layer, forward_batch.out_cache_loc, cache_k, None
                )
            else:
                self.token_to_kv_pool.set_kv_buffer(
        ...
        if meta.is_uncached_prefill_only:
            if k_rope is not None:
                k_attn = torch.cat([k, k_rope], dim=-1)
                v_attn = k
            else:
                k_attn = k
                v_attn = v
            ...
            if k_rope is not None:
                self._raise_unsupported_mla_cache_path("extend")
    ...
    def forward_decode(
        ...
        v_cache = (
            None
            if meta.is_mla
            else self.token_to_kv_pool.get_value_buffer(layer.layer_id)
        )
    ...
    def forward_mixed(
        ...
        if k_rope is not None:
            self._raise_unsupported_mla_cache_path("mixed")
        ...
        if decode_idx:
            ...
            # MLA: v_cache must be None for decode kernel (reads v from combined k_cache)
            v_cache_dec = None if meta.is_mla else v_cache
