...
def _get_mlu_module():
    if not hasattr(torch, "mlu"):
        try:
            ...
            raise RuntimeError("torch_mlu is required for the MLU backend.") from exc
    return torch.mlu
...
class MluDeviceMixin(DeviceMixin):
    ...
    def get_device_total_memory(self, device_id: int = 0) -> int:
        return int(_get_mlu_module().get_device_properties(device_id).total_memory)

    def get_current_memory_usage(
        self, device: Optional["torch.device"] = None
    ) -> float:
        mlu = _get_mlu_module()
        mlu.reset_peak_memory_stats(device)
        return float(mlu.max_memory_allocated(device))

    def get_device(self, local_rank: int = 0) -> "torch.device":
        return torch.device("mlu", local_rank)

    def set_device(self, device: "torch.device") -> None:
        _get_mlu_module().set_device(device)

    def get_device_name(self, device_id: int = 0) -> str:
        return str(_get_mlu_module().get_device_name(device_id))

    def get_device_uuid(self, device_id: int = 0) -> str:
        props = _get_mlu_module().get_device_properties(device_id)
    ...
    def empty_cache(self) -> None:
        _get_mlu_module().empty_cache()

    def synchronize(self) -> None:
        _get_mlu_module().synchronize()

    def get_available_memory(self, device_id: int = 0) -> tuple[int, int]:
        return _get_mlu_module().mem_get_info()
    ...
    def seed_everything(cls, seed: int | None = None) -> None:
        if seed is not None:
            ...
            _get_mlu_module().manual_seed_all(seed)
