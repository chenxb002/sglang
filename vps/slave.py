...
class TaskStore:
    ...
    def read_log(self, task_id: str, tail_lines: Optional[int] = None,
                 start: int = 0) -> str:
...
class SlaveHandler(BaseHTTPRequestHandler):
    ...
    def handle_bridge_get_log(self, params: Dict[str, str], task_id: str) -> None:
        ...
        try:
            ...
            json_response(self, 400, {"status": "error", "error": "start must be an integer"})
            return
        text_response(self, 200, self.store.read_log(task_id, tail_lines=tail or None, start=start))
