...
register_mlu_ci(est_time=900, suite="nightly-1-mlu", nightly=True)
...
class TestMLUQwen314BE2E(CustomTestCase):
    def test_qwen3_14b_server_smoke(self):
        ...
        model_path = Path(
            os.environ.get("MLU_QWEN3_14B_MODEL_PATH", "/data/models/Qwen3-14B/")
        )
        if not model_path.is_dir():
            self.skipTest(f"Qwen3-14B model path does not exist: {model_path}")
