...
class TestResolvePlatformWithEnv(CustomTestCase):
    ...
    @patch("sglang.srt.platforms.envs")
    def test_selected_mlu_uses_in_tree_platform(self, mock_envs, _mock_mlu):
        """SGLANG_PLATFORM=mlu selects the in-tree MLU platform."""
        mock_envs.SGLANG_PLATFORM.get.return_value = "mlu"
        self.assertIsInstance(_resolve_platform(), MluSRTPlatform)

    @patch("sglang.srt.platforms._is_mlu_available", return_value=False)
    @patch("sglang.srt.platforms.envs")
    def test_selected_mlu_unavailable_raises(self, mock_envs, _mock_mlu):
        """SGLANG_PLATFORM=mlu fails loudly when MLU is unavailable."""
        mock_envs.SGLANG_PLATFORM.get.return_value = "mlu"
        with self.assertRaises(RuntimeError):
            _resolve_platform()
